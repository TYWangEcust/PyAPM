...
def element_atomID_dict(fileName, elementsByType):
    ...
    # Ensure elementsByType is uppercase（确保elementsbytype是大写）
    elementsByTypeDict = {index+1: val.upper() for index, val in enumerate(elementsByType)} # Keys: Type, Val: Elements

    # Assert that there are enough types in elementsByType for the highest type in the types variable（假设elementsbytype中有足够的类型满足types变量中的最高类型）
