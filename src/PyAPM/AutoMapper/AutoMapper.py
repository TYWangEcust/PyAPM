...
from AtomSymbolSearch import new_map_processor
# Init the parser（初始化解析器）

parser = argparse.ArgumentParser(description='Run preprocessing tools for LAMMPS simulation using fix bond/react')

# List of arguments for command line（命令行参数列表）
...
parser.add_argument('tool', metavar='tool', type=str, nargs=1, choices=['clean', 'molecule', 'map', 'newmap'], help='Name of tool to be used. Possible tools: clean, molecule, map, newmap')
...
# Get arguments from parser（从解析器获取参数）
args = parser.parse_args()
# Take compulsory args out of list - other args done later（将强制参数从list删除——其他参数稍后完成
tool = args.tool[0]
directory = args.directory[0]


# Throw errors if arguments are missing from certain tools（如果某些工具缺少参数，则抛出错误）
...
# Combined molecule and map creation code（结合分子和map创建代码）
    ...
    new_map_processor(directory, args.data_files[0], args.data_files[1], args.save_name[0], args.save_name[1], args.ba[0], args.ba[1], args.da, args.ebt, args.ca, args.debug)
