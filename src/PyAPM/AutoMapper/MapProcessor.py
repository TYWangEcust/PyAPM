...
def map_processor(directory, preDataFileName, postDataFileName, preMoleculeFileName, postMoleculeFileName, preBondingAtoms, postBondingAtoms, deleteAtoms, elementsByType, createAtoms, debug=False):
    #（文件，反应前后的data文件，分子文件，连键原子，删除原子，原子类型，创建原子）
    # Set log level
    if debug:
    ...
    # Split delete atoms list, if given（拆分'删除原子'列表（如果给定）
    ...
    # Cut map down to smallest possible partial structure（将map缩减到最小的局部结构）
    with restore_dir():
    ...
    # Determine if bonding atom is part of a cycle, and if so what atoms make up the cycle and their neighbours（确定键合原子是否是循环的一部分,如果是的话,哪些原子组成循环及其邻近原子）
    ...
    # Determine atoms to be kept if reaction is a ring opening（如果反应是一个环的开放确定原子被保留）
    prePartialAtomsSet, postPartialAtomsSet = is_ring_opening(prePreservedAtomIDs, postPreservedAtomIDs, mappedIDList)

    # Keep atoms up to 4 bonds away from the bonding atoms（使原子与键合原子保持最多 4 个键的距离
    ...
    # Refind edge atoms after potential extension(势能扩展后重新找到边缘原子）
    preEdgeAtoms = find_edge_atoms(preAtomObjectDict, prePartialAtomsSet)

    # Check for and get byproduct atoms that aren't deleteIDs（检查并获取不是deleteID的副产品原子
    ...
    # Order mappedIDList by preAtomID（按照preAtomID赋值mappedID）
    mappedIDList = natsorted(mappedIDList, key=lambda x: x[0])

    # Create empty partialMappedIDList to fill the return（创建空的partialmapped来填充返回值）
    partialMappedIDList = []

    # Renumber map if the partial structure has a different length to the full structure
    # If they're equal then just output the map, no changes needed（如果部分结构的长度与完整结构的长度不同，则重新编号映射
    if len(prePartialAtomsSet) != len(preAtomObjectDict):
        logging.debug(f'Creating a partial map.')
        # Build a partial map and get the renumbering dictionaries（构建部分map并重新编号）
        ...
        # Renumber key features for molecule creation and output（重新编号分子创建和输出的关键特征
        ...
        # Renumber edge atoms if any given（重现编号边缘原子（如果给定））
        if preEdgeAtoms is not None:
            preEdgeAtoms = renumber(preEdgeAtoms, preRenumberdAtomDict)

        # If create atoms are included, figure out the renumberedAtomDict and then renumber（重新编号新创建的原子）
        if createAtoms is not None:
            IDCounter = max([int(val) for val in postRenumberedAtomDict.values()]) # Initialise counter as highest ID of renumbered atoms（将计数器初始化为重新编号原子的最高 ID）

            # Extend renumberedAtomDict for createAtoms（为创建原子扩展词典）
            for createAtom in createAtoms:
                IDCounter += 1
                postRenumberedAtomDict[createAtom] = str(IDCounter)

            # Renumber createAtoms with the new renumberedAtomDict（在新的原子词典中重新编号新原子）
            createAtoms = renumber(createAtoms, postRenumberedAtomDict)


        # Rebuild molecule files with partial structure（对局部结构重新创建分子文件）
        with restore_dir():
    ...
    # Returns mappedIDList for other functions to use e.g. testing（返回map字典供其他函数使用，例如测试）
...
# Utility for moving to a different os path and then returning to the original directory（用于移动到其他操作系统路径，然后返回到原始目录的实用程序）
...
def bfs(graph, startAtom, endAtom, breakLink=False):
    ...
    # List to track if an atomID has already been seen（根据已经能观察到的原子ID建立连接路径
    discovered = {key: False for key in graph.keys()}

    discovered[startAtom] = True

    newGraph = deepcopy(graph)
     # Break link between start atom and target atom if present - stops search going backwards when searching for cycles（断开起始原子和目标原子的连接——在搜素循环时停止搜素向后进行）
    if breakLink:
        newGraph[startAtom].remove(endAtom)

    # Iterate through paths whilst keeping a record of all paths（遍历路径，同时记录所有路径）
    queue = []
    ...
    while queue:
        ...
        # Get latest path element（得到最新路径）
        node = path[-1]
        ...
        for neighbour in newGraph.get(node, []):
            # Prevents path getting stuck in a loop（避免在循环中卡住）
            if discovered[neighbour]:
                continue
            
            # Increase the path by next neighbour and add to queue（通过下一个邻居增加路径并添加到队列）
...
def is_cyclic(atomObjectDict, bondingAtoms, reactionType):
    # Create dictionary of adjacent bonds - IMPROVEMENT: Remove H from adjacent bonds since they can't go anywhere（创建相邻键的字典 - 改进：从相邻键中删除 H，因为它们不能去任何地方）
